...
# ----------------------------------------------------------------------
# Step 5: hls.c — gate audio-rendition probe on !cur_init_section
# ----------------------------------------------------------------------
# fMP4/CMAF audio renditions (EXT-X-MAP -> cur_init_section set) already
# have codec params + timing in the moov box parsed by
# avformat_open_input(), so the nested per-rendition find_stream_info
# call is redundant. With default 5 MB / 5 s probe caps and ~50 KB / 1 s
# CMAF segments that probe drains ~100 encrypted segments (minutes) per
# rendition before returning. Gate the branch on !pls->cur_init_section
# to keep the original behaviour for genuinely header-less raw audio
# (no init section), which is where the function's existing "raw audio"
# rationale actually applies.
#
# Idempotency token: the literal `!pls->cur_init_section)) {` from the
# patched form.

echo "[firedown] Gating hls.c audio-rendition probe on !cur_init_section..."
...
if '!pls->cur_init_section)) {' in text:
...
insertion = (
    '         * on us if they want to.\n'
    '         */\n'
    '        /* fMP4/CMAF audio renditions (EXT-X-MAP present -> cur_init_section set)\n'
    '         * already have codec params + timing in the moov parsed by\n'
    '         * avformat_open_input() above, so this nested probe is redundant and\n'
    '         * can drain ~100 encrypted segments (minutes) before returning. Only\n'
    '         * run it for genuinely header-less raw audio (no init section). */\n'
    '        av_log(s, AV_LOG_ERROR, "FIREDOWN_V4_ACTIVE seg0=%s n_rend=%d init=%p\\n",\n'
    '               (pls->n_segments > 0 && pls->segments[0]) ? pls->segments[0]->url : "?",\n'
    '               pls->n_renditions, (void*)pls->cur_init_section);\n'
    '        if (pls->is_id3_timestamped ||\n'
    '            (pls->n_renditions > 0 && pls->renditions[0]->type == AVMEDIA_TYPE_AUDIO &&\n'
    '             !pls->cur_init_section)) {\n'
)
...
print('[firedown]   hls.c: audio-rendition probe gated on !cur_init_section')
PYEOF
