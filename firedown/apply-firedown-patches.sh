...
# ----------------------------------------------------------------------
# Step 5: hls.c diagnostic injections (dev only)
# ----------------------------------------------------------------------
# Two related edits, applied in one Python step so the file is read/written
# once:
#
#   (a) Bound the nested per-playlist probe. The audio-rendition path in
#       hls_read_header calls avformat_find_stream_info on pls->ctx; with
#       the default 5 MB / 5 s caps an encrypted CMAF audio rendition will
#       fetch ~100 segments before returning. Cap to 256 KiB / 500 ms.
#       Anchor: the line immediately after pls->ctx flags are OR-merged,
#       inserting right before the ff_copy_whiteblacklists call.
#
#   (b) Log every per-playlist decision in recheck_discard_flags() so we
#       can see which playlists toggle needed=0/1 and why renditions are
#       being dropped or kept. AV_LOG_INFO.
#       Anchor: the two-line block `struct playlist *pls = ...` + blank
#       line + `cur_needed = playlist_needed(...)`.
#
# Idempotency: both edits leave a `[firedown]` token in the C source as a
# marker. A re-run on an already-patched tree sees the token and skips.

echo "[firedown] Injecting hls.c diagnostic edits..."
...
# (a) probe-cap on nested per-playlist contexts
if '[firedown] Bound the nested per-playlist probe' in text:
    pass
else:
    anchor_a = (
    ...
    insertion_a = (
        '        pls->ctx->flags   |= s->flags & ~AVFMT_FLAG_CUSTOM_IO;\n'
        '\n'
        '        /* [firedown] Bound the nested per-playlist probe (the audio-rendition\n'
        '         * find_stream_info below). Defaults of 5 MB / 5 s x ~50 KB segments at\n'
        '         * ~1 s each on encrypted CMAF audio renditions burn ~100 segments per\n'
        '         * rendition before find_stream_info returns. */\n'
        '        pls->ctx->probesize            = 256 * 1024;          /* 256 KiB */\n'
        '        pls->ctx->max_analyze_duration = 500 * 1000;          /* 500 ms */\n'
        '\n'
        '        if ((ret = ff_copy_whiteblacklists(pls->ctx, s)) < 0)\n'
    )
    if anchor_a not in text:
        sys.stderr.write(
            "ERROR: hls.c (a) — anchor not found.\n"
            "       Expected the block:\n"
            "         pls->ctx->flags   |= s->flags & ~AVFMT_FLAG_CUSTOM_IO;\n"
            "         <blank>\n"
            "         if ((ret = ff_copy_whiteblacklists(pls->ctx, s)) < 0)\n"
        ...
        sys.exit(20)
    text = text.replace(anchor_a, insertion_a, 1)
    did.append('hls.c: nested-probe caps (256 KiB / 500 ms)')

# (b) recheck_discard_flags log
if 'RECHECK pls[%d]' in text:
    pass
else:
    anchor_b = (
    ...
    if anchor_b not in text:
        sys.stderr.write(
            "ERROR: hls.c (b) — recheck_discard_flags anchor not found.\n"
            "       Expected:\n"
            "         struct playlist *pls = c->playlists[i];\n"
            "         <blank>\n"
            "         cur_needed = playlist_needed(c->playlists[i]);\n"
            "       Update the anchor in apply-firedown-patches.sh.\n")
        sys.exit(21)
    text = text.replace(anchor_b, insertion_b, 1)
    did.append('hls.c: recheck_discard_flags log')
...
if did:
    ...
    print('[firedown]   hls.c diagnostic edits already applied')
PYEOF
