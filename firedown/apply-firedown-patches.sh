...
#
# Renditions with an init section (fMP4 / CMAF) already have codec
# params and timing in the moov box that avformat_open_input parses,
# so the per-rendition probe adds nothing. Gate the audio-rendition
# branch on !pls->cur_init_section — preserves behaviour for renditions
# without an init section (raw AAC / MPEG-TS / etc., where the
# function's "raw audio" rationale actually applies).
#
# Idempotency: searches for the literal `!pls->cur_init_section)) {`
# that the patched form ends with.
