...
# ----------------------------------------------------------------------
# Step 5: hls.c — skip nested probe entirely for fMP4 sub-demuxers
# ----------------------------------------------------------------------
# A fragmented-MP4 segment carries full codecpar in the init moov and
# per-fragment timestamps in tfdt, so there's nothing for the nested
# avformat_find_stream_info(pls->ctx, NULL) to recover. But the probe
# is real: for renditions whose avg_frame_rate doesn't settle in the
# first frames (e.g. Niconico 360p/480p/720p), it walks segment after
# segment until probesize / analyzeduration expires, downloading much
# of the track during avformat_open_input(). The 144p variant happens
# to settle instantly so the bug is invisible there; higher renditions
# hang.
#
# Probe is only meaningful for raw ADTS/TS audio that lacks
# timestamps. Gate on `!sub_is_mp4` (mov demuxer's name is
# "mov,mp4,m4a,3gp,3g2,mj2", which av_match_name("mp4", ...) catches).
#
# Idempotency token: the literal `FIREDOWN: never run this probe on
# an fMP4` from the injected comment.

echo "[firedown] Skipping hls.c nested probe for fMP4 sub-demuxers..."
...
if 'FIREDOWN: never run this probe on an fMP4' in text:
    print('[firedown]   hls.c fMP4-probe-skip already applied')
    sys.exit(0)
...
insertion = (
    ...
    '         */\n'
    '        /*\n'
    '         * FIREDOWN: never run this probe on an fMP4 (mov) sub-demuxer. A\n'
    '         * fragmented-MP4 segment carries full codecpar in its init moov and\n'
    '         * per-fragment timestamps in tfdt, so there is nothing to recover here\n'
    '         * - but the probe is avformat_find_stream_info() on the sub-demuxer,\n'
    '         * which (for renditions whose avg_frame_rate doesn\'t settle in the\n'
    '         * first frames, e.g. Niconico 360p/480p/720p) walks segment after\n'
    '         * segment until probesize/analyzeduration, downloading much of the\n'
    '         * track during avformat_open_input(). 144p happens to settle instantly\n'
    '         * and is unaffected; the higher renditions hang. This probe is only\n'
    '         * meaningful for raw ADTS/TS audio that lacks timestamps.\n'
    '         */\n'
    '        int sub_is_mp4 = pls->ctx && pls->ctx->iformat &&\n'
    '                         av_match_name("mp4", pls->ctx->iformat->name); /* matches "mov,mp4,m4a,3gp,3g2,mj2" */\n'
    '\n'
    '        if (!sub_is_mp4 &&\n'
    '            (pls->is_id3_timestamped || (pls->n_renditions > 0 && pls->renditions[0]->type == AVMEDIA_TYPE_AUDIO))) {\n'
)
...
print('[firedown]   hls.c: nested probe skipped for fMP4 sub-demuxers')
PYEOF
